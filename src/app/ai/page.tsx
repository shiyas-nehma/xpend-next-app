...
// Client no longer imports the SDK directly to avoid exposing the key. All requests go through /api/ai/chat.
...
const AIPage: React.FC = () => {
    ...
    // Removed direct SDK usage; server handles conversation.
    const [chatHistory, setChatHistory] = useState<Message[]>([]); // local merged history for API payload
    ...
    const generateTitle = async (userMessage: string, modelResponse: string): Promise<string> => {
        // Simple heuristic title generation without another model call to avoid extra tokens.
    ...
    const handleSendMessage = async (messageContent: string) => {
        ...
        try {
            // Call server API (no streaming for now; could be upgraded with edge & streaming later)
